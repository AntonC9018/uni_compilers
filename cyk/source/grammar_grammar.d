...
struct ProductionList
{
    ...
    size_t opDollar()
    {
        return _oneOrMore.children.length / 2 + 1;
    }
    ...
    ProductionSyntax opIndex(size_t index)
    {
        ...
        return ProductionSyntax(_oneOrMore[index - 1][1]);
    }
    ...
    auto opSlice()
    {
        ...
        return only(_first)
            .chain(
                iota(0, _oneOrMore.children.length)
                    .map!(i => _oneOrMore[i][1][0]))
